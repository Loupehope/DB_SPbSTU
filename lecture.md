...
Это мол куда архивируются файлы реду лога
Там мы добавляли новый путь, к еще одной папке
И вот в команде sql к нему есть пункт optional_reopen
=300
300- дефолтное значение
Можно эту функцию отключить
Или поставить ей значени 0, что тоже ее отключает
Сама она отвечает за то, какое минимальное время должнл пройти перед тем, как процесс arc попробует снова получить доступ к этой папке в случае неудачи с первого раза
Чтобы заархивировать туда данные
...
DBMS_DATAPUMP - пакет для перемещения информации и метаданных (expdp и impdp) - старый вариант exp/imp
Direct Path Api - запись в блоки, выше HWM (без sga)
Directory - синоним на папку ос
audit_dir, data_file_dir, data_pump_dir, log_file_dir - преопределенные
grant read/write на директорию
SQL Loader - запись данных в бд из текстовых файлов (sqlld). Используется control file для описания того:
- где лежат данные для загрузки
- формата данных
- Деталей конфигурации: управление памяться, records rejection, условия остановки загрузки
- Детали работы с данными

Conventional Load (до HWM):
- Используется commit
- Генерируются записи отмены
- Проверяются все ограничения
- Триггеры на insert
- Есть возможность записи в кластерные таблицы
- Позволяет пользователям модифицировать таблицу при загрузке

Direct Path Load (после HWM):
- Механизм сохранения данных
- Redo только при определенных условиях
- Проверяется только primers key, unique и not null (нет - check, foreign key)
- Не работают триггеры на insert
- Нет возможности записи в кластерные таблицы
- Не позволяет пользователям модифицировать таблицу при загрузке

Внешние таблицы с Oracle_datapump
Запись из таблицы в двоичные файлы (их нельзя менять)

С ORACLE_LOADER файлы с данными уже существуют, а с Oracle_datapump мы можем их создать и заполнить
Sample - процент данных, который надо выгрузить либо объем работы
REMAP - переназначать фалы данных, табличные пространства, схемы
PARALLEL - число параллельных файлов заполнения

dba_external_tables
dba_external_locations
dba_tables - внешняя или нет
dba_external_tables
