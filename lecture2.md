...
Ручное incomplete db recovery:
1)	Закрыть БД;
2)	restore файлов данных;
3)	смонтировать БД;
4)	recover БД;
5)	открыть БД с resetlogs;   
...
Некритичные файлы: restore + recover.   
Complete recovery: loss System-critical data file in archivelog (system / undo):
1.	shutdown abort если нужно
2.	монтируем БД
3.	restore и recover для потерянного файла данных
4.	открытие БД.   
...
Серверное incomplete recovery (rman):
1)	определение точки: scn, время, restore point, log sequence number
2)	установка nls переменной окружения
3)	монтирование БД
4)	запуск RUN блока, используя set until, restore, recover
5)	открытие бд в режиме read only и проверка, что все окей
6)	открытие БД с resetlogs

Recovery with backup control file:
1)	restore и монтирование бэкапа control file, когда все файлы потеряны или повреждены
2)	выполнение recover 
3)	открытие БД с опцией resetlogs (можно complete или incomplete)
 
Restoring the server PFILE from the control file autobackup.   
Restoring control file from autobackup.   
Использование инкрементальных бэкапов – ограниченное восстановление в режиме noarchivelog:
1) startup force nomount; 
2) restore controlfile;
3) alter database mount;
4) restore database;
5) recover database noredo; (восстановление без информации из archive log, будут использоваться инкрементальные бэкапы)
