...
– OPTIMIZER_MODE: устанавливает подход, который использует оптимизатор для определения наилучшего плана для экземпляра или сессии. Возможные значения – ALL_ROWS, FIRST_ROWS_n (n=1,10,100,1000), FIRST_ROWS. По умолчанию ALL_ROWS
– OPTIMIZER_FEATURES_ENABLE: сдвигает поведение оптимизатора к более ранним версиям, контролируя набор параметров оптимизатора, большинство из которых является недокументированными.
...
- Система автоматически обслуживает индексы, когда изменяется связанная информация.
- Вы можете создать и удалить индекс в любое время. Если вы удаляете индекс, все приложения продолжают работать. Однако, доступ к ранее проиндексированной информации может быть медленнее.
- Индексы могут быть уникальными (unique) или неуникальными (nonunique).
- Композитный (composite) индекс, также называемый составным (concatenated) индексом, - это индекс, который вы создаете на несколько столбцов (до 32) в таблице.
- Столбцы в композитном индексе могут появиться в любом порядке и не должны быть смежными в таблице.
